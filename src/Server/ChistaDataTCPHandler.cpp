...
ChistaDataTCPHandler::ChistaDataTCPHandler(
    IServer & server_,
...
void ChistaDataTCPHandler::runImpl()
{
    setThreadName("ChistaDataTCPHandler");
    ...
    while (tcp_server.isOpen())
    {
        ...
        try
        {
            ...
            query_context->setExternalTablesInitializer(
            ...
            query_context->setInputInitializer(
                [this](ContextPtr context, const StoragePtr & input_storage)
                {
                    if (context != query_context)
            ...
            query_context->setReadTaskCallback(
                [this]() -> String
                {
            ...
            query_context->setMergeTreeReadTaskCallback(
                [this](PartitionReadRequest request) -> std::optional<PartitionReadResponse>
                {
                    std::lock_guard lock(task_callback_mutex);
...
void ChistaDataTCPHandler::extractConnectionSettingsFromContext(const ContextPtr & context)
{
...
bool ChistaDataTCPHandler::readDataNext()
{
...
void ChistaDataTCPHandler::readData()
{
    sendLogs();
...
void ChistaDataTCPHandler::skipData()
{
...
void ChistaDataTCPHandler::processInsertQuery()
{
...
void ChistaDataTCPHandler::processOrdinaryQueryWithProcessors()
{
...
void ChistaDataTCPHandler::processTablesStatusRequest()
{
...
void ChistaDataTCPHandler::receiveUnexpectedTablesStatusRequest()
{
...
void ChistaDataTCPHandler::sendPartUUIDs()
{
...
void ChistaDataTCPHandler::sendReadTaskRequestAssumeLocked()
{
...
void ChistaDataTCPHandler::sendMergeTreeReadTaskRequestAssumeLocked(PartitionReadRequest request)
{
...
void ChistaDataTCPHandler::sendProfileInfo(const ProfileInfo & info)
{
...
void ChistaDataTCPHandler::sendTotals(const Block & totals)
{
    if (totals)
    {
...
void ChistaDataTCPHandler::sendExtremes(const Block & extremes)
{
    if (extremes)
    {
...
void ChistaDataTCPHandler::sendProfileEvents()
{
    Block block;
...
void ChistaDataTCPHandler::sendSelectProfileEvents()
{
    if (client_tcp_protocol_version < DBMS_MIN_PROTOCOL_VERSION_WITH_INCREMENTAL_PROFILE_EVENTS)
        return;
...
void ChistaDataTCPHandler::sendInsertProfileEvents()
{
...
bool ChistaDataTCPHandler::receiveProxyHeader()
{
    if (in->eof())
    {
...
std::unique_ptr<Session> ChistaDataTCPHandler::makeSession()
{
...
void ChistaDataTCPHandler::receiveHello()
{
...
void ChistaDataTCPHandler::receiveAddendum()
{
...
void ChistaDataTCPHandler::receiveUnexpectedHello()
{
    UInt64 skip_uint_64;
    String skip_string;
...
void ChistaDataTCPHandler::sendHello()
{
...
bool ChistaDataTCPHandler::receivePacket()
{
    UInt64 packet_type = 0;
    readVarUInt(packet_type, *in);
...
void ChistaDataTCPHandler::receiveIgnoredPartUUIDs()
{
    readVectorBinary(state.part_uuids_to_ignore.emplace(), *in);
}


void ChistaDataTCPHandler::receiveUnexpectedIgnoredPartUUIDs()
{
...
String ChistaDataTCPHandler::receiveReadTaskResponseAssumeLocked()
{
    UInt64 packet_type = 0;
    readVarUInt(packet_type, *in);
...
std::optional<PartitionReadResponse> ChistaDataTCPHandler::receivePartitionMergeTreeReadTaskResponseAssumeLocked()
{
    UInt64 packet_type = 0;
    readVarUInt(packet_type, *in);
...
void ChistaDataTCPHandler::receiveClusterNameAndSalt()
{
    readStringBinary(cluster, *in);
    readStringBinary(salt, *in, 32);
}

void ChistaDataTCPHandler::receiveQuery()
{
    UInt64 stage = 0;
    ...
    if (is_interserver_mode)
    {
        ...
        auto exception = Exception(
            "Inter-server secret support is disabled, because ClickHouse was built without SSL library", ErrorCodes::AUTHENTICATION_FAILED);
        session->onAuthenticationFailure(/* user_name */ std::nullopt, socket().peerAddress(), exception);
        throw exception; /// NOLINT
#endif
    }
...
void ChistaDataTCPHandler::receiveUnexpectedQuery()
{
    UInt64 skip_uint_64;
    String skip_string;
...
bool ChistaDataTCPHandler::receiveData(bool scalar)
{
    initBlockInput();
...
bool ChistaDataTCPHandler::receiveUnexpectedData(bool throw_exception)
{
...
void ChistaDataTCPHandler::initBlockInput()
{
    if (!state.block_in)
    {
...
void ChistaDataTCPHandler::initBlockOutput(const Block & block)
{
    if (!state.block_out)
    {
...
void ChistaDataTCPHandler::initLogsBlockOutput(const Block & block)
{
    if (!state.logs_block_out)
    {
        ...
        state.logs_block_out = std::make_unique<NativeWriter>(
            *out, client_tcp_protocol_version, block.cloneEmpty(), !query_settings.low_cardinality_allow_in_native_format);
    }
}


void ChistaDataTCPHandler::initProfileEventsBlockOutput(const Block & block)
{
...
bool ChistaDataTCPHandler::isQueryCancelled()
{
...
void ChistaDataTCPHandler::sendData(const Block & block)
{
...
void ChistaDataTCPHandler::sendLogData(const Block & block)
{
...
void ChistaDataTCPHandler::sendTableColumns(const ColumnsDescription & columns)
{
...
void ChistaDataTCPHandler::sendException(const Exception & e, bool with_stack_trace)
{
...
void ChistaDataTCPHandler::sendEndOfStream()
{
...
void ChistaDataTCPHandler::updateProgress(const Progress & value)
{
    state.progress.incrementPiecewiseAtomically(value);
}


void ChistaDataTCPHandler::sendProgress()
{
...
void ChistaDataTCPHandler::sendLogs()
{
    if (!state.logs_queue)
        return;
